...
import com.openwearables.healthsdk.data.entities.DeviceTypeMapper
import com.openwearables.healthsdk.interfaces.HealthDataProvider
import com.openwearables.healthsdk.data.entities.ProviderReadResult
import com.openwearables.healthsdk.data.entities.UnifiedHealthData
import com.openwearables.healthsdk.data.entities.UnifiedRecord
import com.openwearables.healthsdk.data.entities.UnifiedSleep
import com.openwearables.healthsdk.data.entities.UnifiedSource
import com.openwearables.healthsdk.data.entities.UnifiedTimestamp
import com.openwearables.healthsdk.data.entities.UnifiedWorkout
...
class HealthConnectManager(
    ...
    override suspend fun requestAuthorization(typeIds: List<String>): Boolean {
        ...
        if (!::client.isInitialized || !::permissionLauncher.isInitialized) return false
    ...
    private suspend fun readDataInternal(
        ...
        try {
            when (typeId) {
                "steps" -> readRecordType<StepsRecord>(sinceTimestamp, limit, isAscending, olderThanTimestamp = olderThanTimestamp) { convertSteps(it) }
                "heartRate" -> readRecordType<HeartRateRecord>(sinceTimestamp, limit, isAscending, olderThanTimestamp = olderThanTimestamp) { convertHeartRate(it) }
                "restingHeartRate" -> readRecordType<RestingHeartRateRecord>(sinceTimestamp, limit, isAscending, olderThanTimestamp = olderThanTimestamp) { convertRestingHeartRate(it) }
                "heartRateVariabilitySDNN" -> readRecordType<HeartRateVariabilityRmssdRecord>(sinceTimestamp, limit) { convertHrv(it) }
                "oxygenSaturation" -> readRecordType<OxygenSaturationRecord>(sinceTimestamp, limit, isAscending, olderThanTimestamp = olderThanTimestamp) { convertOxygenSaturation(it) }
                "bloodPressure", "bloodPressureSystolic", "bloodPressureDiastolic" -> readRecordType<BloodPressureRecord>(sinceTimestamp, limit, isAscending, olderThanTimestamp = olderThanTimestamp) { convertBloodPressure(it) }
                "bloodGlucose" -> readRecordType<BloodGlucoseRecord>(sinceTimestamp, limit, isAscending, olderThanTimestamp = olderThanTimestamp) { convertBloodGlucose(it) }
                "activeEnergy" -> readRecordType<ActiveCaloriesBurnedRecord>(sinceTimestamp, limit, isAscending, olderThanTimestamp = olderThanTimestamp) { convertActiveCalories(it) }
                "basalEnergy" -> readRecordType<BasalMetabolicRateRecord>(sinceTimestamp, limit, isAscending, olderThanTimestamp = olderThanTimestamp) { convertBasalCalories(it) }
                "bodyTemperature" -> readRecordType<BodyTemperatureRecord>(sinceTimestamp, limit, isAscending, olderThanTimestamp = olderThanTimestamp) { convertBodyTemperature(it) }
                "bodyMass" -> readRecordType<WeightRecord>(sinceTimestamp, limit, isAscending, olderThanTimestamp = olderThanTimestamp) { convertWeight(it) }
                "height" -> readRecordType<HeightRecord>(sinceTimestamp, limit, isAscending, olderThanTimestamp = olderThanTimestamp) { convertHeight(it) }
                "bodyFatPercentage" -> readRecordType<BodyFatRecord>(sinceTimestamp, limit, isAscending, olderThanTimestamp = olderThanTimestamp) { convertBodyFat(it) }
                "leanBodyMass" -> readRecordType<LeanBodyMassRecord>(sinceTimestamp, limit, isAscending, olderThanTimestamp = olderThanTimestamp) { convertLeanBodyMass(it) }
                "flightsClimbed" -> readRecordType<FloorsClimbedRecord>(sinceTimestamp, limit, isAscending, olderThanTimestamp = olderThanTimestamp) { convertFloors(it) }
                "distanceWalkingRunning" -> readRecordType<DistanceRecord>(sinceTimestamp, limit, isAscending, olderThanTimestamp = olderThanTimestamp) { convertDistance(it) }
                "water", "dietaryWater" -> readRecordType<HydrationRecord>(sinceTimestamp, limit, isAscending, olderThanTimestamp = olderThanTimestamp) { convertHydration(it) }
                "vo2Max" -> readRecordType<Vo2MaxRecord>(sinceTimestamp, limit, isAscending, olderThanTimestamp = olderThanTimestamp) { convertVo2Max(it) }
                "respiratoryRate" -> readRecordType<RespiratoryRateRecord>(sinceTimestamp, limit, isAscending, olderThanTimestamp = olderThanTimestamp) { convertRespiratoryRate(it) }
                "distanceCycling" -> readRecordType<DistanceRecord>(sinceTimestamp, limit, isAscending, olderThanTimestamp = olderThanTimestamp) { convertDistance(it) }
    ...
    private fun buildSource(metadata: Metadata): UnifiedSource {
        ...
        return UnifiedSource(
            ...
            recordingMethod = metadata.recordingMethod.let {
                DeviceTypeMapper.fromHealthConnectRecordingMethod(
                    it
                )
            }
        )
    ...
    private fun convertSteps(records: List<StepsRecord>): ProviderReadResult {
        ...
        val unified = records.map { r ->
            ...
            UnifiedRecord(
                r.metadata.id,
                "STEP_COUNT",
                instantToIso(r.startTime),
                instantToIso(r.endTime),
                zoneStr(r.startZoneOffset),
                buildSource(r.metadata),
                r.count.toDouble(),
                "count",
                null,
                null
    ...
    private fun convertHeartRate(records: List<HeartRateRecord>): ProviderReadResult {
        ...
        for (r in records) {
            ...
            for ((idx, sample) in r.samples.withIndex()) {
                ...
                unified.add(
                    UnifiedRecord(
                        "$parentId-s$idx", "HEART_RATE", iso, iso, zo, source,
    ...
    private fun convertHrv(records: List<HeartRateVariabilityRmssdRecord>): ProviderReadResult {
        ...
        val unified = records.map { r ->
            ...
            UnifiedRecord(
                ...
                zoneStr(r.zoneOffset),
                buildSource(r.metadata),
                r.heartRateVariabilityMillis,
                "ms",
                null,
                mapOf("method" to "rmssd")
            )
        }
        return ProviderReadResult(UnifiedHealthData(records = unified), maxTs)
    }
    ...
    private fun convertOxygenSaturation(records: List<OxygenSaturationRecord>): ProviderReadResult {
        ...
        val unified = records.map { r ->
            ...
            val iso = instantToIso(r.time)
            UnifiedRecord(
                r.metadata.id, "OXYGEN_SATURATION", iso, iso, zoneStr(r.zoneOffset),
    ...
    private fun convertBloodPressure(records: List<BloodPressureRecord>): ProviderReadResult {
        ...
        for (r in records) {
            ...
            unified.add(
                UnifiedRecord(
                    "$parentId-sys", "BLOOD_PRESSURE_SYSTOLIC", iso, iso, zo, src,
                    r.systolic.inMillimetersOfMercury, "mmHg", parentId, meta.ifEmpty { null })
            )
            unified.add(
                UnifiedRecord(
                    "$parentId-dia", "BLOOD_PRESSURE_DIASTOLIC", iso, iso, zo, src,
    ...
    private fun convertBasalCalories(records: List<BasalMetabolicRateRecord>): ProviderReadResult {
        ...
        val unified = records.map { r ->
            ...
            UnifiedRecord(
                ...
                zoneStr(r.zoneOffset),
                buildSource(r.metadata),
                r.basalMetabolicRate.inKilocaloriesPerDay,
                "kcal/day",
                null,
                null
            )
    ...
    private fun convertBodyTemperature(records: List<BodyTemperatureRecord>): ProviderReadResult {
        ...
        val unified = records.map { r ->
            ...
            UnifiedRecord(
                r.metadata.id, "BODY_TEMPERATURE", iso, iso, zoneStr(r.zoneOffset),
    ...
    private fun convertWeight(records: List<WeightRecord>): ProviderReadResult {
        ...
        val unified = records.map { r ->
            ...
            val iso = instantToIso(r.time)
            UnifiedRecord(
                r.metadata.id, "WEIGHT", iso, iso, zoneStr(r.zoneOffset),
    ...
    private fun convertHeight(records: List<HeightRecord>): ProviderReadResult {
        ...
        val unified = records.map { r ->
            ...
            val iso = instantToIso(r.time)
            UnifiedRecord(
                r.metadata.id, "HEIGHT", iso, iso, zoneStr(r.zoneOffset),
    ...
    private fun convertBodyFat(records: List<BodyFatRecord>): ProviderReadResult {
        ...
        val unified = records.map { r ->
            ...
            val iso = instantToIso(r.time)
            UnifiedRecord(
                r.metadata.id, "BODY_FAT", iso, iso, zoneStr(r.zoneOffset),
                buildSource(r.metadata), r.percentage.value, "%", null, null
            )
        }
        return ProviderReadResult(UnifiedHealthData(records = unified), maxTs)
    ...
    private fun convertLeanBodyMass(records: List<LeanBodyMassRecord>): ProviderReadResult {
        ...
        val unified = records.map { r ->
            ...
            val iso = instantToIso(r.time)
            UnifiedRecord(
                r.metadata.id, "LEAN_BODY_MASS", iso, iso, zoneStr(r.zoneOffset),
    ...
    private fun convertFloors(records: List<FloorsClimbedRecord>): ProviderReadResult {
        ...
        val unified = records.map { r ->
            ...
            if (maxTs == null || end > maxTs) maxTs = end
            UnifiedRecord(
                r.metadata.id, "FLOORS_CLIMBED", instantToIso(r.startTime), instantToIso(r.endTime),
    ...
    private fun convertRespiratoryRate(records: List<RespiratoryRateRecord>): ProviderReadResult {
        ...
        val unified = records.map { r ->
            ...
            val iso = instantToIso(r.time)
            UnifiedRecord(
                r.metadata.id, "RESPIRATORY_RATE", iso, iso, zoneStr(r.zoneOffset),
    ...
    private fun convertVo2Max(records: List<Vo2MaxRecord>): ProviderReadResult {
        ...
        val unified = records.map { r ->
            ...
            val iso = instantToIso(r.time)
            UnifiedRecord(
                r.metadata.id, "VO2_MAX", iso, iso, zoneStr(r.zoneOffset),
    ...
    private suspend fun readWorkouts(
        ...
        val workouts = response.records.map { r ->
            ...
            val source = buildSource(r.metadata)
            val zo = zoneStr(r.startZoneOffset)
            val duration = Duration.between(r.startTime, r.endTime).seconds.toDouble()
