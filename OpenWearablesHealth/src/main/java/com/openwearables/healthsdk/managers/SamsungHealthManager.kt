...
class SamsungHealthManager(
    ...
    private suspend fun loadConnectedDevices() {
        try {
            ...
            for (group in listOf(DeviceGroup.MOBILE, DeviceGroup.WATCH, DeviceGroup.RING, DeviceGroup.BAND, DeviceGroup.ACCESSORY)) {
                try {
                    deviceManager.getDevices(group).forEach { device -> deviceCache[device.id] = device }
                } catch (_: Exception) {}
            }
    ...
    override suspend fun readData(
        ...
        val rawRecords = readRawData(typeId, sinceTimestamp = sinceTimestamp, limit = limit)
    ...
    override suspend fun readDataDescending(
        ...
        val rawRecords = readRawData(typeId, olderThanTimestamp = olderThanTimestamp, limit = limit)
    ...
    private suspend fun readRawData(
        ...
        getAggregateConfig(typeId)?.let {
            return@withContext readAggregateData(typeId, it, sinceTimestamp, olderThanTimestamp)
        }
        ...
        try {
            ...
            val request = buildReadRequest(dataType, sinceTimestamp, olderThanTimestamp, limit) ?: return@withContext emptyList()
    ...
    private suspend fun readAggregateData(
        ...
        try {
            ...
            try {
                ...
                groupUnitClass.enumConstants?.find { (it as Enum<*>).name == "HOURLY" }?.let { hourlyUnit ->
                    val groupOfMethod =
                        groupClass.getMethod("of", groupUnitClass, Int::class.java)
                    val timeGroup = groupOfMethod.invoke(null, hourlyUnit, 1)
                    builderClass.getMethod(
                        "setLocalTimeFilterWithGroup",
                        LocalTimeFilter::class.java,
                        groupClass
            ...
            try {
                builderClass.getMethod("setOrdering", Ordering::class.java)
                    .invoke(builder, if (olderThanTimestamp == null) Ordering.ASC else Ordering.DESC)
            } catch (_: Exception) {}
    ...
    private fun parseAggregateItem(typeId: String, fieldName: String, item: Any?): HealthDataRecord? {
        ...
        try {
            val methods = item.javaClass.methods.associateBy { it.name }
            ...
            val startMs = when (startTime) {
                is Instant -> startTime.toEpochMilli()
            ...
            val endMs = when (endTime) {
                is Instant -> endTime.toEpochMilli()
    ...
    private fun buildUnifiedSource(raw: HealthDataRecord): UnifiedSource? {
        val source = raw.dataSource ?: return null
    ...
    private fun buildReadRequest(
        ...
        try { builderClass.getMethod("setOrdering", Ordering::class.java)
            .invoke(builder, if (olderThanTimestamp == null) Ordering.ASC else Ordering.DESC) } catch (e: Exception) { logger("[buildReadRequest] setOrdering failed: ${e.message}") }
        ...
        try {
            sinceTimestamp?.let {
                ...
                olderThanTimestamp?.let {
                    ...
                    builderClass.getMethod("setLocalTimeFilter", LocalTimeFilter::class.java).invoke(builder, filter)
                    logger("[buildReadRequest] Applied time filter before=$endTime")
                }
            }
    ...
    private fun <T> getFieldValue(dataType: DataType, fieldName: String, dataPoint: HealthDataPoint): T? {
        return try {
            ...
            val getValueMethod = dataPoint.javaClass.getMethod("getValue", Field::class.java)
