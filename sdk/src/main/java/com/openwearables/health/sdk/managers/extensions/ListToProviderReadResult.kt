package com.openwearables.health.sdk.managers.extensions
...
// ---- Hydration (L → mL) ----
@JvmName("HydrationMap")
fun List<HydrationRecord>.mapToProvider(): ProviderReadResult {
    var maxTs: Long? = null
    val unified = this.map { r ->
...
// ---- Steps ----
@JvmName("StepsMap")
fun List<StepsRecord>.mapToProvider(): ProviderReadResult {
    var maxTs: Long? = null
    val unified = this.map { r ->
        val end = r.endTime.toEpochMilli()
        if (maxTs == null || end > maxTs) maxTs = end
...
@JvmName("HeartBeatMap")
fun List<HeartRateRecord>.mapToProvider(): ProviderReadResult {
    var maxTs: Long? = null
...
// ---- Resting Heart Rate ----
@JvmName("RestingMap")
fun List<RestingHeartRateRecord>.mapToProvider(): ProviderReadResult {
    var maxTs: Long? = null
    val unified = this.map { r ->
...
// ---- HRV ----
@JvmName("HeartBeatVarMap")
fun List<HeartRateVariabilityRmssdRecord>.mapToProvider(): ProviderReadResult {
    var maxTs: Long? = null
    val unified = this.map { r ->
        val ts = r.time.toEpochMilli()
        if (maxTs == null || ts > maxTs) maxTs = ts
...
// ---- Oxygen Saturation ----
@JvmName("OxygenSatMap")
fun List<OxygenSaturationRecord>.mapToProvider(): ProviderReadResult {
    var maxTs: Long? = null
    val unified = this.map { r ->
        val ts = r.time.toEpochMilli()
        if (maxTs == null || ts > maxTs) maxTs = ts
...
@JvmName("BloodPressureMap")
fun List<BloodPressureRecord>.mapToProvider(): ProviderReadResult {
    var maxTs: Long? = null
    val unified = mutableListOf<UnifiedRecord>()
    for (r in this) {
        val ts = r.time.toEpochMilli()
...
// ---- Blood Glucose ----
@JvmName("BloodGlucoseMap")
fun List<BloodGlucoseRecord>.mapToProvider(): ProviderReadResult {
    var maxTs: Long? = null
    val unified = this.map { r ->
        val ts = r.time.toEpochMilli()
        if (maxTs == null || ts > maxTs) maxTs = ts
...
// ---- Active Calories ----
@JvmName("ActiveCaloriesMap")
fun List<ActiveCaloriesBurnedRecord>.mapToProvider(): ProviderReadResult {
    var maxTs: Long? = null
    val unified = this.map { r ->
        val end = r.endTime.toEpochMilli()
        if (maxTs == null || end > maxTs) maxTs = end
...
@JvmName("BasalMetabolicMap")
fun List<BasalMetabolicRateRecord>.mapToProvider(): ProviderReadResult {
    var maxTs: Long? = null
    val unified = this.map { r ->
        val ts = r.time.toEpochMilli()
        if (maxTs == null || ts > maxTs) maxTs = ts
...
// ---- Body Temperature ----
@JvmName("BodyTempMap")
fun List<BodyTemperatureRecord>.mapToProvider(): ProviderReadResult {
    var maxTs: Long? = null
    val unified = this.map { r ->
        val ts = r.time.toEpochMilli()
        if (maxTs == null || ts > maxTs) maxTs = ts
...
// ---- Weight ----
@JvmName("WeightMap")
fun List<WeightRecord>.mapToProvider(): ProviderReadResult {
    var maxTs: Long? = null
    val unified = this.map { r ->
        val ts = r.time.toEpochMilli()
        if (maxTs == null || ts > maxTs) maxTs = ts
...
// ---- Height ----
@JvmName("HeightMap")
fun List<HeightRecord>.mapToProvider(): ProviderReadResult {
    var maxTs: Long? = null
    val unified = this.map { r ->
        val ts = r.time.toEpochMilli()
        if (maxTs == null || ts > maxTs) maxTs = ts
...
// ---- Body Fat ----
@JvmName("BodyFatMap")
fun List<BodyFatRecord>.mapToProvider(): ProviderReadResult {
    var maxTs: Long? = null
    val unified = this.map { r ->
        val ts = r.time.toEpochMilli()
        if (maxTs == null || ts > maxTs) maxTs = ts
...
// ---- Lean Body Mass ----
@JvmName("LeanBodyMassMap")
fun List<LeanBodyMassRecord>.mapToProvider(): ProviderReadResult {
    var maxTs: Long? = null
    val unified = this.map { r ->
        val ts = r.time.toEpochMilli()
        if (maxTs == null || ts > maxTs) maxTs = ts
...
// ---- Floors Climbed ----
@JvmName("FloorsMap")
fun List<FloorsClimbedRecord>.mapToProvider(): ProviderReadResult {
    var maxTs: Long? = null
    val unified = this.map { r ->
        val end = r.endTime.toEpochMilli()
        if (maxTs == null || end > maxTs) maxTs = end
...
// ---- Distance ----
@JvmName("DistanceMap")
fun List<DistanceRecord>.mapToProvider(): ProviderReadResult {
    var maxTs: Long? = null
    val unified = this.map { r ->
        val end = r.endTime.toEpochMilli()
        if (maxTs == null || end > maxTs) maxTs = end
...
// ---- Respiratory Rate ----
@JvmName("RespiratoryRateMap")
fun List<RespiratoryRateRecord>.mapToProvider(): ProviderReadResult {
    var maxTs: Long? = null
    val unified = this.map { r ->
        val ts = r.time.toEpochMilli()
        if (maxTs == null || ts > maxTs) maxTs = ts
...
// ---- VO2 Max ----
@JvmName("Vo2MaxMap")
fun List<Vo2MaxRecord>.mapToProvider(): ProviderReadResult {
    var maxTs: Long? = null
    val unified = this.map { r ->
        val ts = r.time.toEpochMilli()
        if (maxTs == null || ts > maxTs) maxTs = ts
...
// Workouts
@JvmName("ExerciseSessionMap")
fun List<ExerciseSessionRecord>.mapToProvider(): ProviderReadResult {
    var maxTs: Long? = null
    val workouts = this.map { r ->
        val end = r.endTime.toEpochMilli()
        if (maxTs == null || end > maxTs) maxTs = end
        val source = buildSource(r.metadata)
        val zo = zoneStr(r.startZoneOffset)
...
// Sleep
@JvmName("SleepSessionMap")
fun List<SleepSessionRecord>.mapToProvider(): ProviderReadResult {
    var maxTs: Long? = null
    ...
    for (r in this) {
        try {
            ...
            val stages = try { r.stages } catch (_: Exception) {
                emptyList()
            }
